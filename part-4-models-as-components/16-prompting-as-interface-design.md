...
Chapter 15 showed that model behavior depends on context and generation mechanics.

Now we turn the prompt into an engineering interface. The prompt is not a spell. It is the contract between software and model.
...
Maybe. But better wording is not enough.
...
- human review rule is missing.

This is weak interface design.
...
- refusal rule is unclear;
- output validation is absent;
- no eval set exists.
...
Next chapter chooses which model or route should serve the interface. One model for every task becomes slow, expensive, and brittle.
