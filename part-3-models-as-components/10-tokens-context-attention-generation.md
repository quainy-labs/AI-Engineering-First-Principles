# Chapter 10: Tokens, Context, Attention, and Generation

## Real Problem

Your RAG assistant gives worse answers after you add more documents to context.

You expected more context to improve answer.
...
Because language model does not "read documents" like human. It processes tokens inside limited context, attends across them, then generates next tokens. Context design matters.

## Bad Default Solution

Bad solution:

> Put everything in prompt.
...
Context window is maximum tokens model can consider in one request.
...
If context window is full, system must choose what to include and what to remove.

### Attention

Attention lets model relate tokens to other tokens in context.

For engineering, key lesson:

> Model behavior depends on what context is present, where it appears, and how clear its relevance is.

Attention is powerful but not guarantee. Important evidence can be diluted by irrelevant context.
...
This is why small changes in context can change answer.
...
- retrieved sources from model-generated text;
...
Conversation history: only current ticket, not full user history
...
- Does answer cite included evidence?
- Does answer ignore irrelevant distractors?
- Does answer refuse when evidence missing?
- Does answer follow authority order?
- Does latency fit budget?
- Does cost fit budget?

Create 10 context stress tests:
