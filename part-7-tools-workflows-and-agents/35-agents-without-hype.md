...
Chapter 34 completed the action foundations: interfaces, tools, workflow, state, approval, and computer use.
...
Next chapter evaluates agents by trace. A useful agent must prove not only final result, but path quality, tool use, safety, and stop behavior.
