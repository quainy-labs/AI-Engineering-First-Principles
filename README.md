# AI Engineering from First Principles

**Build Reliable Intelligent Systems That Improve Real Work**

> Take a messy real-world workflow, decide whether AI is useful, design a reliable intelligent system, build it, evaluate it, deploy it responsibly, and improve it with feedback.

## Reader Promise

After finishing this book, reader should be able to:

- analyze a real workflow;
- decide when not to use AI;
- design system boundaries;
- organize data, documents, and state;
- use models as components;
- build grounded retrieval systems;
- connect AI to tools and workflows;
- evaluate quality with real examples;
- reason about cost, latency, security, and risk;
- ship proof of useful capability.

## Book Shape

- 6 parts.
- 24 chapters.
- 6 serious projects.
- 1 capstone.

Each chapter ends with a proof artifact. No chapter ends with only "understanding."

## Research & Idea

[AI Engineering Blueprint](AI_ENGINEERING_BOOK_BLUEPRINT.md)

## Projects

[Project Guide](projects/README.md)

## Table of Contents

### Part 1: Problem-First AI Engineering

Capability: decide what to build and why.

1. [AI Engineering Is System Design](part-1-problem-first-ai-engineering/01-ai-engineering-is-system-design.md)
2. [When Not to Use AI](part-1-problem-first-ai-engineering/02-when-not-to-use-ai.md)
3. [From Messy Workflow to System Boundary](part-1-problem-first-ai-engineering/03-from-messy-workflow-to-system-boundary.md)
4. [Metrics, Baselines, and Meaningful Impact](part-1-problem-first-ai-engineering/04-metrics-baselines-meaningful-impact.md)

Project: [Support Workflow Intelligence Console](projects/01-workflow-teardown.md)

### Part 2: Information Foundations

Capability: turn messy knowledge into usable system context.

5. [Data, Documents, and State](part-2-information-foundations/05-data-documents-and-state.md)
6. [Schemas, Contracts, and Structured Outputs](part-2-information-foundations/06-schemas-contracts-structured-outputs.md)
7. [Search, Ranking, and Information Architecture](part-2-information-foundations/07-search-ranking-information-architecture.md)
8. [Embeddings and Semantic Retrieval](part-2-information-foundations/08-embeddings-semantic-retrieval.md)

Project: [Knowledge Base Search Engine](projects/02-knowledge-base.md)

### Part 3: Models as Components

Capability: use models with understanding, not worship.

9. [What Language Models Actually Do](part-3-models-as-components/09-what-language-models-actually-do.md)
10. [Tokens, Context, Attention, and Generation](part-3-models-as-components/10-tokens-context-attention-generation.md)
11. [Prompting as Interface Design](part-3-models-as-components/11-prompting-as-interface-design.md)
12. [Model Failure Modes and Error Analysis](part-3-models-as-components/12-model-failure-modes-error-analysis.md)

Project: [Structured Intake Extractor](projects/03-structured-extraction-system.md)

### Part 4: Grounded AI Systems

Capability: build systems that answer from trusted knowledge.

13. [Retrieval-Augmented Generation](part-4-grounded-ai-systems/13-retrieval-augmented-generation.md)
14. [Context Engineering and Grounding](part-4-grounded-ai-systems/14-context-engineering-grounding.md)
15. [RAG Evaluation](part-4-grounded-ai-systems/15-rag-evaluation.md)
16. [From Assistant Demo to Knowledge Product](part-4-grounded-ai-systems/16-from-assistant-demo-to-knowledge-product.md)

Project: [Grounded Research Assistant](projects/04-grounded-research-assistant.md)

### Part 5: Tools, Workflows, and Agents

Capability: build AI systems that take bounded action.

17. [Tool Use and API Actions](part-5-tools-workflows-agents/17-tool-use-api-actions.md)
18. [Workflow Orchestration](part-5-tools-workflows-agents/18-workflow-orchestration.md)
19. [Memory, State, and Human Approval](part-5-tools-workflows-agents/19-memory-state-human-approval.md)
20. [Agents Without Hype](part-5-tools-workflows-agents/20-agents-without-hype.md)

Project: [Operations Action Assistant](projects/05-operations-assistant.md)

### Part 6: Reliable Production AI

Capability: ship systems people can trust.

21. [Evaluation as Product Infrastructure](part-6-reliable-production-ai/21-evaluation-as-product-infrastructure.md)
22. [Observability, Cost, and Latency](part-6-reliable-production-ai/22-observability-cost-latency.md)
23. [Security, Privacy, and Risk Management](part-6-reliable-production-ai/23-security-privacy-risk-management.md)
24. [Deployment, Feedback, and Continuous Improvement](part-6-reliable-production-ai/24-deployment-feedback-continuous-improvement.md)

Project: [Production AI Control Center](projects/06-production-hardening.md)

## Capstone

[Build One Real Intelligent System](capstone.md)
